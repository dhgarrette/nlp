package dhg.nlp.hmm.tag

import dhg.nlp.util.CollectionUtils._
import dhg.util.CollectionUtil._
import dhg.nlp.hmm.tag.TagDict._
...
sealed trait TagDict[Sym, Tag] {
  ...
  final def allTags: Set[Tag] = setIterator.flatMap(_._2).toSet ++ defaultSet

  /** The number of entries in the tag dictionary */
  final def size: Int = setIterator.sumBy(_._2.size)
}
...
object SimpleTagDict {
  ...
  def apply[Sym, Tag](d: Map[Sym, Set[Tag]], default: Set[Tag]): TagDict[Sym, Tag] = UniformWeightedTagDict(d, default)
}
